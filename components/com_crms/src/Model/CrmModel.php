...
namespace Joomla\Component\Crms\Site\Model;
...
/**
 * Crms Component Crm Model
 *
 * @since  1.5
 */
class CrmModel extends ItemModel
{
	/**
	 ...
	 * @since   1.6
	 */
	protected $_context = 'com_crms.crm';
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_crms')) && (!$user->authorise('core.edit', 'com_crms')))
		{
	...
	/**
	 * Method to get crm data.
	 *
	 * @param   integer  $pk  The id of the crm.
	 *
	...
	{
		$pk = (int) $pk ?: (int) $this->getState('crm.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if ($data === null)
				{
					throw new \Exception(Text::_('COM_CRMS_ERROR_FEED_NOT_FOUND'), 404);
				}
	...
	/**
	 * Increment the hit counter for the crm.
	 *
	...
	public function hit($pk = 0)
	{
		...
		if ($hitcount)
		{
			$pk = (!empty($pk)) ? $pk : (int) $this->getState('crm.id');

			$table = $this->getTable('Crm', 'Administrator');
			$table->hit($pk);
		}
