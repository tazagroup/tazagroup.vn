 ...
 * @since  1.6
 */
class DaotaoModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.daotao';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.daotaos', 'daotaos', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.daotaos.data', array());
		...
		$this->preprocessData('com_hrms.daotaos', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_daotao'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_daotao'));
				$db->setQuery($query);
				$max = $db->loadResult();
