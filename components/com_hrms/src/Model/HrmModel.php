...
/**
 * Hrms Component Hrm Model
 *
 * @since  1.5
 */
class HrmModel extends ItemModel
{
	/**
	 ...
	 * @since   1.6
	 */
	protected $_context = 'com_hrms.hrm';
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_hrms')) && (!$user->authorise('core.edit', 'com_hrms')))
		{
	...
	/**
	 * Method to get hrm data.
	 *
	 * @param   integer  $pk  The id of the hrm.
	 *
	...
	{
		$pk = (int) $pk ?: (int) $this->getState('hrm.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if ($data === null)
				{
					throw new \Exception(Text::_('COM_HRMS_ERROR_FEED_NOT_FOUND'), 404);
				}
	...
	/**
	 * Increment the hit counter for the hrm.
	 *
	...
	public function hit($pk = 0)
	{
		...
		if ($hitcount)
		{
			$pk = (!empty($pk)) ? $pk : (int) $this->getState('hrm.id');

			$table = $this->getTable('Hrm', 'Administrator');
			$table->hit($pk);
		}
