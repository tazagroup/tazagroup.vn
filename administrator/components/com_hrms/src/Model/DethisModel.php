...
class DethisModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					...
					$db->quoteName('a.idTLN'),
					$db->quoteName('a.Tendethi'),
					$db->quoteName('a.MaDT'),
		...
		)->select([$db->quoteName('u.Tenchude', 'Tenchude')])
        ->from($db->quoteName('#__hrms_dethi', 'a'))
        ->join('LEFT', $db->quoteName('#__hrms_chude', 'u'), $db->quoteName('u.id') . ' = ' . $db->quoteName('a.idChude'));
