 ...
 * @since  1.6
 */
class LichhopModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.lichhop';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.lichhops', 'lichhops', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.lichhops.data', array());
		...
		$this->preprocessData('com_hrms.lichhops', $data);
...
//			$data['published'] = 0;
//		}
//        if (isset($data['idThamgia']) && is_array($data['idThamgia']))
//		{
//			$registry = new Registry($data['idThamgia']);
//			$data['idThamgia'] = (string) $registry;
//		}
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_lichhop'))
	...
	public function getItem($pk = null)
	{
		...
		{
//            $registry = new Registry($item->idThamgia);
//			$item->idThamgia = $registry->toArray();
		}
		return $item;
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_lichhop'));
				$db->setQuery($query);
				$max = $db->loadResult();
