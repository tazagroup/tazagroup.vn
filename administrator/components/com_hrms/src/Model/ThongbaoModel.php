...
class ThongbaoModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.thongbao';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.thongbaos', 'thongbaos', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.thongbaos.data', array());
		...
		$this->preprocessData('com_hrms.thongbaos', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_thongbao'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_thongbao'));
				$db->setQuery($query);
				$max = $db->loadResult();
