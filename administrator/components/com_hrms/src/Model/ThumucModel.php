 ...
 * @since  1.6
 */
class ThumucModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.thumuc';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.thumucs', 'thumucs', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.thumucs.data', array());
...
//			if ($this->getState('thumuc.id') == 0)
//			{
//				$app = Factory::getApplication();
//				$data->set('catid', $app->input->get('catid', $app->getUserState('com_hrms.thumucs.filter.category_id'), 'int'));
//			}
		}

		$this->preprocessData('com_hrms.thumucs', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_thumuc'))
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('COUNT(*)')
					->from($db->quoteName('#__hrms_thumuc'))
                    ->where([$db->quoteName('pid')."=".$db->quote($table->pid),$db->quoteName('LoaiTM')."=".$db->quote($table->LoaiTM)]);
				$db->setQuery($query);
				$max = $db->loadResult();
