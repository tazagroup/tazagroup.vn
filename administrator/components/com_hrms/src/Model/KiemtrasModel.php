...
class KiemtrasModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				[
			 $db->quoteName('a.idLop'),
             $db->quoteName('a.idHV'),
             $db->quoteName('a.Diem'),
             $db->quoteName('a.Loai'),
             $db->quoteName('a.Batdau'),
             $db->quoteName('a.Ketthuc'),
             $db->quoteName('a.HVtraloi'),
             $db->quoteName('a.GVCham'),
        ...
        ->from($db->quoteName('#__hrms_kiemtra', 'a'));
      //  ->join('LEFT', $db->quoteName('#__users', 'u'), $db->quoteName('u.id') . ' = ' . $db->quoteName('a.idGiao'))
       // ->join('LEFT', $db->quoteName('#__users', 'u1'), $db->quoteName('u1.id') . ' = ' . $db->quoteName('a.idNhan'));
        ...
        if ($idLop = (int) $this->getState('filter.idLop'))
		{
			$query->where($db->quoteName('a.idLop') . ' = :idLop')
				->bind(':idLop', $idLop, ParameterType::INTEGER);
		}      
        if ($idHV = (int) $this->getState('filter.idHV'))
		{
			$query->where($db->quoteName('a.idHV') . ' = :idHV')
				->bind(':idHV', $idHV, ParameterType::INTEGER);
		}
    ...
    public function getItems()
	{
		...
		{
			$item->typeAlias = 'com_hrms.kiemtra';
