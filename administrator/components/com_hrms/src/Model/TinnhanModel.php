 ...
 * @since  1.6
 */
class TinnhanModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.tinnhan';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.tinnhans', 'tinnhans', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.tinnhans.data', array());
...
//			if ($this->getState('tinnhan.id') == 0)
//			{
//				$app = Factory::getApplication();
//				$data->set('catid', $app->input->get('catid', $app->getUserState('com_hrms.tinnhans.filter.category_id'), 'int'));
//			}
		}

		$this->preprocessData('com_hrms.tinnhans', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_tinnhan'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_tinnhan'));
				$db->setQuery($query);
				$max = $db->loadResult();
