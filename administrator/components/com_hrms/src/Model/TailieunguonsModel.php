...
class TailieunguonsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					$db->quoteName('a.id'),
					$db->quoteName('a.idChude'),
					$db->quoteName('a.idTG'),
					$db->quoteName('a.idGTG'),
					$db->quoteName('a.idRoot'),
					$db->quoteName('a.Tentailieu'),
					$db->quoteName('a.MaTL'),
					$db->quoteName('a.Lienket'),
					$db->quoteName('a.Ghichu'),
					$db->quoteName('a.Mota'),
					$db->quoteName('a.Trangthai'),
					$db->quoteName('a.TTTailieu'),
					$db->quoteName('a.TTHieuluc'),
					$db->quoteName('a.Tags'),
					$db->quoteName('a.DKTK'),
					$db->quoteName('a.Deadline'),
					$db->quoteName('a.Ngayhieuluc'),
					$db->quoteName('a.published'),
					$db->quoteName('a.idDuyet'),
					$db->quoteName('a.Kiemduyet'),
        ...
        ->from($db->quoteName('#__hrms_tailieunguon', 'a'));
		...
		if ($access = (int) $this->getState('filter.access'))
		{
 		...
 		if ($idChude = (int) $this->getState('filter.idChude'))
		{
			$query->where($db->quoteName('a.idChude') . ' = :idChude')
				->bind(':idChude', $idChude, ParameterType::INTEGER);
		}       
    ...
    public function getItems()
	{
		...
		{
			$item->typeAlias = 'com_hrms.tailieunguon';
