...
class KiemtraModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.kiemtra';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.kiemtras', 'kiemtras', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.kiemtras.data', array());
		...
		$this->preprocessData('com_hrms.kiemtras', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_kiemtra'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_kiemtra'));
				$db->setQuery($query);
				$max = $db->loadResult();
