...
class TailieusModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		)->from($db->quoteName('#__hrms_tailieu', 'a'));
		...
		// Filter by category.
		$idTM = $this->getState('filter.idTM');

		if (is_numeric($idTM))
		{
			$idTM = (int) $idTM;
			$query->where($db->quoteName('a.idTM') . ' = :idTM')
				->bind(':idTM', $idTM, ParameterType::INTEGER);
		}
