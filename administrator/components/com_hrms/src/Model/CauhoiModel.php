 ...
 * @since  1.6
 */
class CauhoiModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.cauhoi';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.cauhois', 'cauhois', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.cauhois.data', array());
...
//			if ($this->getState('cauhoi.id') == 0)
//			{
//				$app = Factory::getApplication();
//				$data->set('catid', $app->input->get('catid', $app->getUserState('com_hrms.cauhois.filter.category_id'), 'int'));
//			}
		}

		$this->preprocessData('com_hrms.cauhois', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_cauhoi'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_cauhoi'));
				$db->setQuery($query);
				$max = $db->loadResult();
