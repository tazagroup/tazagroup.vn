...
class BaihocModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.baihoc';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.baihocs', 'baihocs', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.baihocs.data', array());
		...
		$this->preprocessData('com_hrms.baihocs', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_baihoc'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_baihoc'));
				$db->setQuery($query);
				$max = $db->loadResult();
