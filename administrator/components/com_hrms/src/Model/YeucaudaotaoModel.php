...
class YeucaudaotaoModel extends AdminModel
{
	...
	public $typeAlias = 'com_hrms.yeucaudaotao';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.yeucaudaotaos', 'yeucaudaotaos', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.yeucaudaotaos.data', array());
		...
		$this->preprocessData('com_hrms.yeucaudaotaos', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_yeucaudaotao'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_yeucaudaotao'));
				$db->setQuery($query);
				$max = $db->loadResult();
