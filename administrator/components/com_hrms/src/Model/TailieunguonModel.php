...
class TailieunguonModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.tailieunguon';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.tailieunguons', 'tailieunguons', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.tailieunguons.data', array());
		...
		$this->preprocessData('com_hrms.tailieunguons', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_tailieunguon'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_tailieunguon'))
