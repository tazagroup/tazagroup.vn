...
class LophocsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					$db->quoteName('a.id'),
					$db->quoteName('a.Tenlop'),
					$db->quoteName('a.MaLop'),
					$db->quoteName('a.LinkMeet'),
					$db->quoteName('a.idGV'),
					$db->quoteName('a.idHV'),
					$db->quoteName('a.idBH'),
		...
		)->select([$db->quoteName('u1.idTL', 'idTL'),$db->quoteName('u.idRoot', 'idRoot')])
        ->from($db->quoteName('#__hrms_lophoc', 'a'))
         ->join('LEFT', $db->quoteName('#__hrms_baihoc', 'u1'), $db->quoteName('u1.id') . ' = ' . $db->quoteName('a.idBH'))         
        ->join('LEFT', $db->quoteName('#__hrms_tailieunguon', 'u'), $db->quoteName('u.id') . ' = ' . $db->quoteName('u1.idTL'));
        ...
        if ($MaLop = $this->getState('filter.MaLop'))
		{
			$query->where($db->quoteName('a.MaLop') . ' = :MaLop')
				->bind(':MaLop', $MaLop);
		}       
    ...
    public function getItems()
	{
		...
		{
			$item->typeAlias = 'com_hrms.lophoc';
