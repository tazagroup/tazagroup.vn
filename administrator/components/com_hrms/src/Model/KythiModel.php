...
class KythiModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.kythi';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.kythis', 'kythis', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.kythis.data', array());
		...
		$this->preprocessData('com_hrms.kythis', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_kythi'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_kythi'));
				$db->setQuery($query);
				$max = $db->loadResult();
