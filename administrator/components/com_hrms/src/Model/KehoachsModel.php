 ...
 * @since  1.6
 */
class KehoachsModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					$db->quoteName('a.id'),
					$db->quoteName('a.idUser'),
					$db->quoteName('a.Noidung'),
					$db->quoteName('a.Muctieu'),
					$db->quoteName('a.Thuchien'),
		...
		)->select([$db->quoteName('u.name', 'User')])
        ->from($db->quoteName('#__hrms_kehoach', 'a'))
        ->join('LEFT', $db->quoteName('#__users', 'u'), $db->quoteName('u.id') . ' = ' . $db->quoteName('a.idUser'));
