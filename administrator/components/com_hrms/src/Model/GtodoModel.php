...
class GtodoModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.gtodo';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.gtodos', 'gtodos', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.gtodos.data', array());
		...
		$this->preprocessData('com_hrms.gtodos', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_gtodo'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_gtodo'))
                    ->where($db->quoteName('idCty')."=".$db->quote($table->idCty));
				$db->setQuery($query);
				$max = $db->loadResult();
