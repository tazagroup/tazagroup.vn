...
class NganhangcauhoiModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.nganhangcauhoi';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.nganhangcauhois', 'nganhangcauhois', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.nganhangcauhois.data', array());
		...
		$this->preprocessData('com_hrms.nganhangcauhois', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_nganhangcauhoi'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_nganhangcauhoi'))
                    ->where($db->quoteName('idTL')."=".$db->quote($table->idTL));
				$db->setQuery($query);
				$max = $db->loadResult();
