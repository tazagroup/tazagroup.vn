...
class LophocModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.lophoc';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.lophocs', 'lophocs', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.lophocs.data', array());
		...
		$this->preprocessData('com_hrms.lophocs', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_lophoc'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_lophoc'));
				$db->setQuery($query);
				$max = $db->loadResult();
