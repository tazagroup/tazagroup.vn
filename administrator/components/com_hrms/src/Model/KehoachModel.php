 ...
 * @since  1.6
 */
class KehoachModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.kehoach';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.kehoachs', 'kehoachs', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.kehoachs.data', array());
...
//			if ($this->getState('kehoach.id') == 0)
//			{
//				$app = Factory::getApplication();
//				$data->set('catid', $app->input->get('catid', $app->getUserState('com_hrms.kehoachs.filter.category_id'), 'int'));
//			}
		}

		$this->preprocessData('com_hrms.kehoachs', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_kehoach'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_kehoach'));
				$db->setQuery($query);
				$max = $db->loadResult();
