...
class QuytrinhdaotaosModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
            ...
            $db->quoteName('a.idVitri'),
             ...
             $db->quoteName('a.Mota'),
             $db->quoteName('a.TGDT'),
             $db->quoteName('a.LichDT'),
             $db->quoteName('a.idHV'),
             $db->quoteName('a.idDoituong'),
        ...
        ->from($db->quoteName('#__hrms_quytrinhdaotao', 'a'));
     ...
     // ->join('LEFT', $db->quoteName('#__users', 'u1'), $db->quoteName('u1.id') . ' = ' . $db->quoteName('a.idNhan'));
        ...
        if ($idVitri = (int) $this->getState('filter.idVitri'))
		{
			$query->where($db->quoteName('a.idVitri') . ' = :idVitri')
				->bind(':idVitri', $idVitri, ParameterType::INTEGER);
		}
    ...
    public function getItems()
	{
		...
		{
			$item->typeAlias = 'com_hrms.quytrinhdaotao';
