 ...
 * @since  1.6
 */
class HrmsModel extends ListModel
{
	/**
	 * Constructor.
	 *
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('asso1.context') . ' = ' . $db->quote('com_hrms.item'),
					]
				);
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('type_alias') . ' = ' . $db->quote('com_hrms.hrm'),
					]
				);
			...
			$query->join(
				...
				->where(
					[
						...
						$db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_hrms.hrm'),
					]
				)
