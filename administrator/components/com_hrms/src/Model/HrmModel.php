 ...
 * @since  1.6
 */
class HrmModel extends AdminModel
{
	 ...
	 * @since    3.4.4
	 */
	protected $associationsContext = 'com_hrms.item';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.hrm', 'hrm', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.hrm.data', array());
		...
		if (empty($data))
		{
			...
			if ($this->getState('hrm.id') == 0)
			{
				$app = Factory::getApplication();
				$data->set('catid', $app->input->get('catid', $app->getUserState('com_hrms.hrms.filter.category_id'), 'int'));
			}
		}

		$this->preprocessData('com_hrms.hrm', $data);
	...
	public function save($data)
	{
		...
		{
			$createCategory = !CategoriesHelper::validateCategoryId($data['catid'], 'com_hrms');
		}
	...
	public function getItem($pk = null)
	{
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = Associations::getAssociations('com_hrms', '#__hrms', 'com_hrms.item', $item->id);
		...
		if (!empty($item->id))
		{
			...
			$item->tags->getTagIds($item->id, 'com_hrms.hrm');
		...
		if (Associations::isEnabled())
		{
			...
			if (count($languages) > 1)
			{
				...
				foreach ($languages as $language)
				{
					...
					$field->addAttribute('type', 'modal_hrm');
	...
	{
		return Factory::getUser()->authorise('core.create', 'com_hrms');
	}
