 ...
 * @since  1.6
 */
class TodosModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					...
					$db->quoteName('a.idNhomviec'),
					$db->quoteName('a.idLich'),
					$db->quoteName('a.idGiao'),
					$db->quoteName('a.idNhan'),
		...
		)->select([$db->quoteName('u.name', 'Nguoigiao'),$db->quoteName('u.idKhoi', 'Khoi'),$db->quoteName('u.idPhongban', 'Phongban'),$db->quoteName('u.idVitri', 'Vitri'),$db->quoteName('u.idBophan', 'Bophan'),$db->quoteName('u1.name', 'Nguoinhan')])
        ->from($db->quoteName('#__hrms_todo', 'a'))
