 ...
 * @since  1.6
 */
class LotrinhModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.lotrinh';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.lotrinhs', 'lotrinhs', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.lotrinhs.data', array());
		...
		$this->preprocessData('com_hrms.lotrinhs', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_lotrinh'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_lotrinh'));
				$db->setQuery($query);
				$max = $db->loadResult();
