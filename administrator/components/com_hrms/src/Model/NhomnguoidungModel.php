...
class NhomnguoidungModel extends AdminModel
{
	use VersionableModelTrait;
	public $typeAlias = 'com_hrms.nhomnguoidung';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.nhomnguoidungs', 'nhomnguoidungs', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.nhomnguoidungs.data', array());
		...
		$this->preprocessData('com_hrms.nhomnguoidungs', $data);
...
//        if (isset($data['Phanquyen']) && is_array($data['Phanquyen']))
//		{
//			$registry = new Registry($data['Phanquyen']);
//			$data['Phanquyen'] = (string) $registry;
//		}
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_nhomnguoidung'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_nhomnguoidung'));
				$db->setQuery($query);
				$max = $db->loadResult();
