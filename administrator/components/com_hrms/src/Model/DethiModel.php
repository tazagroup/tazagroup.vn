 ...
 * @since  1.6
 */
class DethiModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_hrms.dethi';
	...
	{
		// Get the form.
		$form = $this->loadForm('com_hrms.dethis', 'dethis', array('control' => 'jform', 'load_data' => $loadData));
...
//		}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_hrms.edit.dethis.data', array());
...
//			if ($this->getState('dethi.id') == 0)
//			{
//				$app = Factory::getApplication();
//				$data->set('catid', $app->input->get('catid', $app->getUserState('com_hrms.dethis.filter.category_id'), 'int'));
//			}
		}

		$this->preprocessData('com_hrms.dethis', $data);
  	...
  	public function delete(&$pks)
	{
		...
		if ($return)
		{
			...
			$query = $db->getQuery(true)
				->delete($db->quoteName('#__hrms_dethi'))
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__hrms_dethi'))
					 ->where($db->quoteName('idChude')."=".$db->quote($table->idChude));
				$db->setQuery($query);
				$max = $db->loadResult();
