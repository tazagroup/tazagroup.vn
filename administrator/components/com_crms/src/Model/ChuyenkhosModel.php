 ...
 * @since  1.6
 */
class ChuyenkhosModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					$db->quoteName('a.id'),
					$db->quoteName('a.idCNChuyen'),
					$db->quoteName('a.idCNNhan'),
					$db->quoteName('a.idNVL'),
					$db->quoteName('a.SoluongChuyen'),
					$db->quoteName('a.PhieuChuyen'),
					$db->quoteName('a.KiemDuyet'),
					$db->quoteName('a.NguoiDuyet'),
		...
		)->select([$db->quoteName('b.TenSP', 'TenSP'),$db->quoteName('c.Soluong', 'Soluong'),$db->quoteName('c.Trangthai', 'Trangthai')])
        ->from($db->quoteName('#__crms_chuyenkho', 'a'))
        ->join('LEFT', $db->quoteName('#__crms_nguyenvatlieu', 'b'), $db->quoteName('b.id') . ' = ' . $db->quoteName('a.idNVL'))
         ->join('LEFT', $db->quoteName('#__crms_chuyenkho_chitiet', 'c'), $db->quoteName('c.idChuyen') . ' = ' . $db->quoteName('a.id'));
