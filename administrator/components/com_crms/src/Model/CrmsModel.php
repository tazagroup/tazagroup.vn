 ...
 * @since  1.6
 */
class CrmsModel extends ListModel
{
	/**
	 * Constructor.
	 *
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('asso1.context') . ' = ' . $db->quote('com_crms.item'),
					]
				);
			...
			$subQuery = $db->getQuery(true)
				...
				->where(
					[
						...
						$db->quoteName('type_alias') . ' = ' . $db->quote('com_crms.crm'),
					]
				);
			...
			$query->join(
				...
				->where(
					[
						...
						$db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_crms.crm'),
					]
				)
