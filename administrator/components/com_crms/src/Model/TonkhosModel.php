 ...
 * @since  1.6
 */
class TonkhosModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = 'a.id', $direction = 'DESC')
	{
		...
		$PhieuXuat = $this->getUserStateFromRequest($this->context . '.filter.PhieuXuat', 'filter_tonkhos');
	...
	protected function getListQuery()
	{
		...
		)->select([$db->quoteName('b.qrcode', 'qrcode'),$db->quoteName('b.HanSD', 'HanSD'),$db->quoteName('d.TenSP', 'TenSP'),$db->quoteName('d.id', 'idNVL'),$db->quoteName('c.name', 'Nguoitao'),$db->quoteName('e.DVT', 'DVT')])
        ->from($db->quoteName('#__crms_tonkho', 'a'))
        ...
        if ($idNhap = (int) $this->getState('filter.idNhap'))
		{    
			$query->where($db->quoteName('a.idNhap') . ' = :idNhap')
				->bind(':idNhap', $idNhap, ParameterType::INTEGER);  
		} 
        if ($qrcode = (string) $this->getState('filter.qrcode'))
		{    		
            $query->where($db->quoteName('b.qrcode') . ' = :qrcode')
				->bind(':qrcode', $qrcode, ParameterType::STRING);  
		}     
