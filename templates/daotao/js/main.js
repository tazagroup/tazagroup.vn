...
app.filter('FDAHV', function () {
  return function (DAHV, data) {
    var result = '';
    angular.forEach(data, function (v) {
      if (parseInt(v.id) === parseInt(DAHV)) {
        result = v.value;
      }
    })
    return result;
...
app.filter('FCheckDanhgia', function () {
  return function (input, data) {
    var result = 0;
    result = (data.find(result => result.id === input) != undefined) ? 1 : 0;
    return result;
  };
});
...
app.filter('Fchecknx', function () {
  return function (input, data) {
    var result = 0;
     if(data!==undefined) 
         {result = (data.find(result => result.gv === input) != undefined) ? 1 : 0;}
    return result;
  };
});
...
//    }
    ...
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/caidats?page[offset]=0&page[limit]=*", {
      ...
      .then(function (res) {
        ...
        $scope.ListTags = (data.find(result => result.id === 29)).Dulieu;
        $scope.ListLoaiKythi = (data.find(result => result.id === 28)).Dulieu;
      });
...
$scope.OninitFixbug = function () {
  ...
  $scope.CreateFixbug = function (dulieu) {
    ...
    console.log(data);
    $http.post("https://tazagroup.vn/api/index.php/v1/hrms/fixbugs", data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
        $scope.OninitFixbug();
        Thongbao(0, 'Tạo Mới Thành Công');
      }, function (res) {
        console.log(res);
        Thongbao(1, geterror(res));
      });
  }
  $scope.ReadFixbug = function () {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/fixbugs?filter[published]=0&page[offset]=0&page[limit]=*", {
        headers: $scope.headers
      })
      .then(function (res) {
        $scope.RAFixbug = [];
        ...
        angular.forEach(data, function (v) {
          ...
          $scope.RAFixbug.push(v);
        });
       // console.log($scope.RAFixbug);
      });

  }
  $scope.UpdateFixbug = function (dulieu) {
    var data = {"Trangthai": 1};
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/fixbugs/" + dulieu.id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.DeleteFixbug = function (dulieu) {
    var data = {
      "published": 1
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/fixbugs/" + dulieu.id, data, {
  ...
  $scope.TrangThaiBug = function (id, Trangthai) {
    var data = {
      "Trangthai": Trangthai,
    };
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/fixbugs/" + id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.ReadListNhanVien = function () {
    $http.get("https://tazagroup.vn/api/index.php/v1/users?page[offset]=0&page[limit]=*", {
      ...
      .then(function (res) {
        ...
        var data = getdata(res.data.data);
        angular.forEach(data, function (v) {
            //console.log(JSON.parse(v.Profile)['Vitri']); 
          let x = ($filter('Ftitle')(JSON.parse(v.Profile)['Vitri'], $scope.Vitri) == '') ? '' : '(' + $filter('Ftitle')(JSON.parse(v.Profile)['Vitri'], $scope.Vitri) + ')';
          v.name = v.name + x;
  ...
  $scope.SetPDF = function (data) {
    $("#iframe_div embed").remove();
    $timeout(function () {
      var file_src = data.flink + '#toolbar=0&navpanes=0&scrollbar=0';
      $('<embed>').attr('src', file_src).attr('height', '700px').attr('width', '100%').attr('toolbar', '0').appendTo('#iframe_div');
      $scope.FilePDF.Title = data.name;
    }, 500)
  }
  $scope.FScreenCheck = true;
  $scope.FScreen = 'xl';
  $scope.ChangeFull = function () {
    $scope.FScreenCheck = !$scope.FScreenCheck;
    $scope.FScreen = ($scope.FScreenCheck == true) ? 'xl' : 'fullscreen';
  }
  ...
  $scope.LocPDF = function (data) {
    let filter = [];
    angular.forEach(data, function (v) {
      if (v.Lienket.length != 0) {
        filter.push(v);
      }
    });
    $scope.RATailieu = filter;
  }


  $scope.editTailieu = function (data) {
    $scope.isedit = data.id;
  ...
  $scope.copyCauhoi = function (data) {
    $scope.Cauhoi = angular.copy(data);
    $scope.Cauhoi.Cauhoi = '';
    $scope.Cauhoi.Capdo = data.Capdo.toString();
    $scope.Cauhoi.MaCH = '';
    $scope.Cauhoi.Traloi = {};
    $scope.Cauhoi.Dapan = {};
    $scope.Cauhoi.CRUD = 0;
  ...
  $scope.ReadNhomnguoidung = function () {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/nhomnguoidungs?filter[published]=0&page[offset]=0&page[limit]=*", {
      ...
      .then(function (res) {
        ...
        $scope.NhomGV = $scope.RNhomnguoidung.find(result => result.id === 8).idNguoidung;
  ...
  $scope.LoadLophoc = function (id) {
    ...
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/lophocs?filter[MaLop]=" + Malop + "&page[offset]=0&page[limit]=*", {
      ...
      .then(function (res) {
        ...
        data.Nhanxet = JSON.parse(data.Nhanxet) || [];
  ...
  $scope.FCheckDanhgia = function (id, data) {
    //    console.log(id, data);
    let x = data.find(result => result.id === id);
    $scope.CheckDanhgia = (x === undefined) ? '0' : x;
    //    console.log($scope.CheckDanhgia);
  }

  $scope.CreadNhanxet = function (gv, hv, nx, lophoc) {
    // console.log(gv, hv, nx, lophoc);
    let x = lophoc.idHV.find(result => result.id === hv).Nhanxet;
    // console.log(x);
    x.push({
      gv: gv,
      nx: nx
    });
    let newidHV = angular.copy(lophoc.idHV).map(obj => obj.id === hv ? {
      ...obj,
      Nhanxet: x,
    } : obj);
    console.log(newidHV);
    ...
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/lophocs/" + lophoc.id, data, {
      ...
      .then(function (res) {
        Thongbao(0, "Nhận Xét Thành Công");
        $scope.OninitLoadLophoc(gv);
      }, function (res) {
        console.log(res);
      });
  }
  ...
  $scope.Danhgialophoc = function (sao, review, lophoc, id) {
    if (sao < 4 && review === undefined) {
      ...
      var data = {
        "idHV": JSON.stringify(newidHV),
        "Danhgia": JSON.stringify(angular.copy(lophoc.Danhgia))
      };
      console.log(data);
      $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/lophocs/" + lophoc.id, data, {
          headers: $scope.headers
  ...
  $scope.HoanthanhKTLH = function (dulieu, hv, lophoc) {
    ...
    var data = {
      ...
      //    "Batdau": $filter('date')(dulieu.Batdau, "yyyy-MM-dd HH:mm:ss"),
      //    "Ketthuc": $filter('date')(dulieu.Ketthuc, "yyyy-MM-dd HH:mm:ss"),
      "HVtraloi": JSON.stringify(angular.copy(dulieu.Traloi)),
      "idTao": $scope.idUser
    };
  ...
  $scope.ReadKiemtra = function (idLop) {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/kiemtras?filter[published]=0&filter[idLop]=" + idLop + "&page[offset]=0&page[limit]=*", {
      ...
      .then(function (res) {
        ...
        var data = getdata(res.data.data);
        angular.forEach(data, function (v) {
          v.HVtraloi = JSON.parse(v.HVtraloi);
          v.GVCham = JSON.parse(v.GVCham);
  ...
  $scope.VChamdiem = function (hv) {
    //console.log(hv);
    $scope.BailamHV = [];
    let x = $scope.RKiemtra.find(result => result.idHV === hv.id).HVtraloi;
...
//        console.log(res);
  ...
  $scope.UpdateTTDuyetKT = function (id, Trangthai) {
    var data = {
      "Trangthai": Trangthai,
    };
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/kythis/" + id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
...
chart.render();
  ...
  $scope.OninitDashboard = function (idUser) {
    ...
    $scope.ReadNhomnguoidung();
    $timeout(function () {
      $scope.LoadHSLophoc(idUser);
      $scope.LoadHSKythi(idUser);
    }, 1500)
    $scope.HosoLophoc = [];
  }
  //Dashboard
  ...
  $scope.CreateYeucauDT = function (dulieu) {  
    var data = {
     "idYeucau":$scope.idUser,
      "BPDDT": dulieu.BPDDT,
      "HVTD": JSON.stringify(dulieu.HVTD),
      "YCDR": dulieu.YCDR,     
      "GVDX": JSON.stringify(dulieu.GVDX),
      "TGDTDK": JSON.stringify(dulieu.GVDX),
      "TGDTDK": $filter('date')(dulieu.TGDTDK, "yyyy-MM-dd HH:mm:ss"),
      "TGDTTT": $filter('date')(dulieu.TGDTTT, "yyyy-MM-dd HH:mm:ss"),      
      "TTGDTDK": $filter('date')(dulieu.TTGDTDK, "yyyy-MM-dd HH:mm:ss"),
      "TTGDTTT": $filter('date')(dulieu.TTGDTTT, "yyyy-MM-dd HH:mm:ss"),
      "HTDT": dulieu.HTDT,  
      "TraNDDT": dulieu.TraNDDT,  
      "CPDK": dulieu.CPDK,  
      "Danhgia": dulieu.Danhgia,  
      "Ghichu": dulieu.Ghichu,  
      "idDuyet": JSON.stringify(dulieu.idDuyet),
      "idTao": $scope.idUser
    };
    console.log(data);
    $http.post("https://tazagroup.vn/api/index.php/v1/hrms/yeucaudaotaos", data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
        $scope.OninitYeucauDT();
        Thongbao(0, 'Tạo Đề Thi Thành Công');
      }, function (res) {
        console.log(res);
        Thongbao(1, geterror(res));
  ...
  $scope.ReadYeucauDT = function () {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/yeucaudaotaos?filter[published]=0&page[offset]=0&page[limit]=*", {
        headers: $scope.headers
      })
      .then(function (res) {
        $scope.RYeucauDT = [];
        ...
        angular.forEach(data, function (v) {
          v.idDuyet = JSON.parse(v.idDuyet) || [];
          v.HVTD = JSON.parse(v.HVTD) || [];
          v.GVDX = JSON.parse(v.GVDX) || [];
          v.TGDTDK = new Date(v.TGDTDK);
          v.TGDTTT = new Date(v.TGDTTT);
          v.TTGDTDK = new Date(v.TTGDTDK);
          v.TTGDTTT = new Date(v.TTGDTTT);
          v.Ngaytao = new Date(v.Ngaytao);
          v.Trangthai = v.Trangthai.toString();
          v.Ngaytao = new Date(v.Ngaytao);
          if (v.Trangthai === 2) {
            $scope.RYeucauDT.push(v);
          }
          $scope.RAYeucauDT.push(v);
          $scope.YeucauDTgoc.push(v);
        });
        // $scope.FidTLCauhoi = $filter('unique')($scope.RACauhoi, 'idTL');    
        ($scope.localStorage.TabDT == 0) ? $scope.Phantrang($scope.RYeucauDT, $scope.localStorage.SLitem, $scope.localStorage.Chontrang): $scope.Phantrang($scope.RAYeucauDT, $scope.localStorage.SLitem, $scope.localStorage.Chontrang);
      });

  }
  $scope.UpdateYeucauDT = function (dulieu) {
    var data = { 
      "idYeucau":$scope.idUser,
      "BPDDT": dulieu.BPDDT,
      "HVTD": JSON.stringify(dulieu.HVTD),
    ...
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/yeucaudaotaos/" + dulieu.id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.DeleteYeucauDT = function (dulieu) {
    var data = {
      "published": 1
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/yeucaudaotaos/" + dulieu.id, data, {
   ...
   $scope.UpdateDuyetYeucau = function (id, Trangthai) {
    var data = {
      "Trangthai": Trangthai,
    };
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/yeucaudaotaos/" + id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.resetYeucauDT = function () {
    $scope.OninitYeucauDT();
  }
  $scope.addYeucauDT = function () {
    $scope.YeucauDT.CRUD = 1;
  }
  $scope.editYeucauDT = function (data) {
    $scope.ResetCauhoi();
    $timeout(function () {
      $scope.YeucauDT = data;
      $scope.YeucauDT.CRUD = 2;
      //      console.log(data);
      angular.forEach(data.idCH, (v) => {
        var x = $scope.Cauhoigoc.find(result => result.id === v.idCH);
        x.Diem = v.Diem;
  ...
  $scope.removeYeucauDT = function (data) {
    $scope.YeucauDT = data;
    $scope.YeucauDT.CRUD = 0;
  }
  $scope.Locyeucaudt = function (data, t) {
    $scope.Store('CDDTActive', t);
    //console.log(t);  
    $scope.RAYeucauDT = $scope.YeucauDTgoc;
    //console.log($scope.RACauhoi);  
    var x = [];
    x.push(data);
    var y = flatten(x).map(el => el.id);
    var Loc = [];
    angular.forEach(y, function (v1) {
      angular.forEach($scope.RAYeucauDT, function (v2) {
        if (parseInt(v2.idChude) == parseInt(v1)) {
          Loc.push(v2);
        }
      });
    });
    $scope.RAYeucauDT = Loc;
    $scope.Store('CDDTHientai', data);
  ...
  $scope.UpdateTTDuyetYC = function (id, Trangthai) {
    var data = {
      "Trangthai": Trangthai,
    };
    console.log(data);
  ...
  //Uploadfile 
  $scope.RemoveUpload = function (data, file, index) {
    // console.log(data.Lienket,index,data.Lienket.splice(index, 1));
    let x = data.Lienket.filter(item => item !== file);
    console.log(data.Lienket, x, index);
    var dulieu = {
      "Lienket": JSON.stringify(x)
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/tailieunguons/" + data.id, dulieu, {
        headers: $scope.headers
      })
      .then(function (res) {
        // console.log(res);
        $http.post("https://tazagroup.vn/index.php?option=com_hrms&task=Tailieu.RemoveUpload&format=raw", file, {
            headers: $scope.headers
          })
          .then(function (res) {
            //console.log(res);
  ...
  // Hoso Begin
  $scope.OninitHoso = function (idUser) {
    $scope.Oninit();
  ...
  $scope.RLHSLH = [];
  $scope.RLHSKT = [];
  $scope.LoadHSLophoc = function (idUser) {
    let Events = [];
    angular.forEach($scope.RALophoc, function (v) {
      angular.forEach(v.idHV, function (v1) {
        if (v1.id == idUser) {
        $scope.RLHSLH.push(v);  
          Events.push({
            id: v.id,
     ...
     console.log($scope.RLHSLH); 
    var calendarEl = document.getElementById('HSLophoc');
  ...
  $scope.LichLophocClick = function (data) {
    console.log(data);
    $scope.HosoLophoc = data.event.extendedProps.data;
    var Modallichhop = new bootstrap.Modal(document.getElementById('HosoLophoc'), {
      keyboard: false,
      backdrop: 'static'
    });
    Modallichhop.show();
  ...
  $scope.LoadHSKythi = function (idUser) {
    let Events = [];
    angular.forEach($scope.RAKythi, function (v) {
      angular.forEach(v.idHV, function (v1) {
        if (v1.id == idUser) {
        $scope.RLHSKT.push(v);   
          Events.push({
    ...
    //console.log($scope.RAKythi);
    console.log(Events);
    var calendarEl = document.getElementById('HSKythi');
  ...
  $scope.LichKythiClick = function (data) {
    console.log(data);
    $scope.HosoKythi = data.event.extendedProps.data;
    var Modallichhop = new bootstrap.Modal(document.getElementById('HosoKythi'), {
      keyboard: false,
      backdrop: 'static'
    });
    Modallichhop.show();
  ...
  $scope.CreateHoso = function (dulieu) {
    var data = {
      "Tenhoso": dulieu.Chude,
      "idChude": dulieu.idChude,
      "idCH": JSON.stringify(dulieu.idCH),
      "idDuyet": JSON.stringify(dulieu.idDuyet),
      "Ghichu": dulieu.Ghichu,
    ...
    console.log(data);
    $http.post("https://tazagroup.vn/api/index.php/v1/hrms/hosos", data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.ReadHoso = function () {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/hosos?filter[published]=0&page[offset]=0&page[limit]=*", {
        headers: $scope.headers
      })
      .then(function (res) {
        $scope.RHoso = [];
        ...
        angular.forEach(data, function (v) {
          ...
          v.Trangthai = v.Trangthai.toString();
          v.Ngaytao = new Date(v.Ngaytao);
          v.Tenhoso = v.Tenchude + '- Đề Số ' + v.ordering;
          if (v.Trangthai === 2) {
            $scope.RHoso.push(v);
          }
          $scope.RAHoso.push(v);
          $scope.Hosogoc.push(v);
        });
        // $scope.FidTLCauhoi = $filter('unique')($scope.RACauhoi, 'idTL');    
        ($scope.localStorage.TabDT == 0) ? $scope.Phantrang($scope.RHoso, $scope.localStorage.SLitem, $scope.localStorage.Chontrang): $scope.Phantrang($scope.RAHoso, $scope.localStorage.SLitem, $scope.localStorage.Chontrang);
      });

  }
  $scope.UpdateHoso = function (dulieu) {
    var data = {
      "Tenhoso": dulieu.Chude,
      "idChude": dulieu.idChude,
      "idCH": JSON.stringify(dulieu.idCH),
    ...
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/hosos/" + dulieu.id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.DeleteHoso = function (dulieu) {
    var data = {
      "published": 1
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/hosos/" + dulieu.id, data, {
  ...
  $scope.resetHoso = function () {
    $scope.OninitHoso();
  }
  $scope.addHoso = function () {
    $scope.Hoso.CRUD = 1;
  }
  $scope.editHoso = function (data) {
    $scope.ResetCauhoi();
    $timeout(function () {
      $scope.Hoso = data;
      $scope.Hoso.CRUD = 2;
  ...
  $scope.removeHoso = function (data) {
    $scope.Hoso = data;
    $scope.Hoso.CRUD = 0;
  }
  $scope.Lochoso = function (data, t) {
    $scope.Store('CDHSActive', t);
    //console.log(t);  
    $scope.RAHoso = $scope.Hosogoc;
    //console.log($scope.RACauhoi);  
    var x = [];
    x.push(data);
    var y = flatten(x).map(el => el.id);
    var Loc = [];
    angular.forEach(y, function (v1) {
      angular.forEach($scope.RAHoso, function (v2) {
        if (parseInt(v2.idChude) == parseInt(v1)) {
          Loc.push(v2);
        }
      });
    ...
    $scope.RAHoso = Loc;
    $scope.Store('CDHSHientai', data);

  }
  $scope.ResetHoso = function () {
    $scope.RemoveStorage('CDHSHientai');
    $scope.RemoveStorage('CDHSActive');
    $scope.ReadHoso();

  }
  $scope.UpdateTTDuyetHS = function (id, Trangthai) {
    var data = {
      "Trangthai": Trangthai,
    };
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/hosos/" + id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
...
// Quytrinh Begin
  $scope.Doituongnhanvien = [{id:1,Title:'Nhân Viên Mới'},{id:2,Title:'Nhân Viên Cũ'}];
  $scope.OninitQuytrinh = function () {
    $scope.Oninit();
      console.log($scope.idVitri);
      $scope.idVitri = $scope.idVitri!='undefined'?$scope.idVitri:$scope.Vitri[0].id;
     $timeout(function () {
     $scope.ReadQuytrinh($scope.idVitri);    
     $scope.Quytrinh = {
    'Title':"Tạo Mới Quy Trình",
    'idVitri':$scope.Vitri[0].id,
    'TenVitri':$scope.Vitri[0].Thuoctinh,  
    'CRUD': 0
    };
     }, 500) 
  ...
  $scope.CreateQuytrinh = function (dulieu) {  
      console.log(dulieu); 
    var data = {
      ...
      "Mota": dulieu.Mota,
      "TGDT": dulieu.TGDT,
      "LichDT": dulieu.LichDT,
      "idHV": JSON.stringify(dulieu.idHV),
      "idDoituong": JSON.stringify(dulieu.idDoituong),
      "idDuyet": JSON.stringify(dulieu.idDuyet),
      "Ghichu": dulieu.Ghichu,
      "idTao": $scope.idUser
    };
    console.log(data);
    $http.post("https://tazagroup.vn/api/index.php/v1/hrms/quytrinhdaotaos", data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
        $scope.OninitQuytrinh();
        Thongbao(0, 'Tạo Thành Công');
      }, function (res) {
        console.log(res);
        Thongbao(1, geterror(res));
  ...
  $scope.ReadQuytrinh = function (idVitri) {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/quytrinhdaotaos?filter[published]=0&filter[idVitri]="+idVitri+"&page[offset]=0&page[limit]=*", {
        headers: $scope.headers
      })
      .then(function (res) {
        $scope.RAQuytrinh = [];
        ...
        angular.forEach(data, function (v) {
          v.Trangthai = v.Trangthai.toString();
          v.Ngaytao = new Date(v.Ngaytao);
          v.TGDT = new Date(v.TGDT);
          v.LichDT = new Date(v.LichDT);
          v.idDuyet = JSON.parse(v.idDuyet)||[];
          v.idHV = JSON.parse(v.idHV)||[];
          $scope.RAQuytrinh.push(v);
        });
     // console.log($scope.RAQuytrinh);   
        ($scope.localStorage.TabDT == 0) ? $scope.Phantrang($scope.RQuytrinh, $scope.localStorage.SLitem, $scope.localStorage.Chontrang): $scope.Phantrang($scope.RAQuytrinh, $scope.localStorage.SLitem, $scope.localStorage.Chontrang);
      });

  }
  $scope.UpdateQuytrinh = function (dulieu) {
    var data = {
      "idVitri": dulieu.idVitri,
      "Noidung": dulieu.Noidung,
      "Mota": dulieu.Mota,
    ...
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/quytrinhdaotaos/" + dulieu.id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.DeleteQuytrinh = function (dulieu) {
    var data = {
      "published": 1
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/quytrinhdaotaos/" + dulieu.id, data, {
   ...
   $scope.SelectQuytrinh = function (item) {
     $scope.idVitri = item.id;
     $scope.Quytrinh.TenVitri = item.Thuoctinh;
       $scope.ReadQuytrinh(item.id);   
       console.log(item); 
  }
  $scope.resetQuytrinh = function () {
    $scope.OninitQuytrinh();
  }
  $scope.addQuytrinh = function () {
    $scope.Quytrinh.CRUD = 1;
  }
  $scope.editQuytrinh = function (data) {
    $scope.Quytrinh = data;
    $scope.Quytrinh.Title = "Cập Nhật Quy Trình";
    $scope.Quytrinh.CRUD = 2;
        console.log(data);

  }
  $scope.removeQuytrinh = function (data) {
    $scope.Quytrinh = data;
    $scope.Quytrinh.CRUD = 0;
  }
  $scope.Locquytrinh = function (data, t) {
    $scope.Store('CDDTActive', t);
    //console.log(t);  
    $scope.RAQuytrinh = $scope.Quytrinhgoc;
    //console.log($scope.RACauhoi);   
    var x = [];
    x.push(data);
    var y = flatten(x).map(el => el.id);
    var Loc = [];
    angular.forEach(y, function (v1) {
      angular.forEach($scope.RAQuytrinh, function (v2) {
        if (parseInt(v2.idChude) == parseInt(v1)) {
          Loc.push(v2);
        }
      });
    });
    $scope.RAQuytrinh = Loc;
    $scope.Store('CDDTHientai', data);
  ...
  $scope.UpdateDuyetQuytrinh = function (id, Trangthai) {
    var data = {
      "Trangthai": Trangthai,
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/quytrinhdaotaos/" + id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
