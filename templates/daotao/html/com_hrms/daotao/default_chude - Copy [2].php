<div class="chude" ng-init="ReadChude()"> 
        <div class="card text-white bg-primary mb-3" ng-if="Quyen.pq9">
        <div class="card-header p-2">
<!--            <i class="fas fa-sync-alt mx-3" ng-click="OninitTailieu();ReadTailieu()"></i>-->
            <span class="ms-2">Danh Sách Chủ Đề</span> <i class="fas fa-plus-circle ms-2" data-bs-toggle="modal" data-bs-target="#addchude" ng-click="prcheck=true"></i></div>
      </div>
      <script type="text/ng-template" id="categoryTree">
    <div class="accordion" id="accordionparent-{{$index+1}}" ng-if="category.children">
        <div ng-repeat="category in category.children" class="{{$index+1}} accordion-item row">
            <h2 class="accordion-header col" ng-click="t['CD'+category.id]=!t['CD'+category.id];LocTLN(category,t,'0')">
              <button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#a{{category.id}}"  ng-class="{'collapsed': !localStorage.CDActive['CD'+category.id]}">
              <span class="me-2 badge rounded-pill bg-warning">{{category.id |FCChudecon:RChude}}</span> <span class="me-2 badge rounded-pill bg-info">{{category|FSTailieu:RTailieu}}</span>{{category.Toc}}. {{category.Tenchude}} 
          </button>
            </h2>
...
<ul class="dropdown-menu">
            ...
            <div id="a{{category.id}}" class="accordion-collapse collapse" data-bs-parent="#accordionparent-{{$index+1}}" ng-class="{'show': localStorage.CDActive['CD'+category.id]}">
      ...
      <div class="accordion" id="accordionparent">
        <div ng-repeat="category in categories" class="{{$index+1}} accordion-item row">
          <h2 class="accordion-header col" ng-click="t['CD'+category.id]=!t['CD'+category.id];LocTLN(category,t,'1')" ng-model="t">
            <button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#a{{category.id}}"  ng-class="{'collapsed': !localStorage.CDActive['CD'+category.id]}"><span class="me-2 badge rounded-pill bg-warning">{{category.id |FCChudecon:RChude}}</span> <span class="me-2 badge rounded-pill bg-info">{{category|FSTailieu:RTailieu}}</span>{{category.Toc}}. {{category.Tenchude}} 
              </button>
          </h2>
          ...
          <div id="a{{category.id}}" class="accordion-collapse collapse" data-bs-parent="#accordionparent" ng-class="{'show': localStorage.CDActive['CD'+category.id]}">
            <div class="accordion-body" ng-include="'categoryTree'">{{category.Tenchude}} </div>
          </div>
        </div>
      </div></div>
