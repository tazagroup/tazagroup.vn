<div class="chude" ng-init="ReadChude()"> 
    ...
    <div class="card-header p-2 d-flex">
        ...
        <span class="me-2 my-auto badge rounded-pill bg-info">{{Dethigoc.length}}</span>
        <span class="ms-2"> Danh Sách Chủ Đề</span> 
        <i class="fas fa-sync-alt mx-3 my-auto" ng-click="ResetDethi()"></i>
    ...
    <div class="accordion" id="accordionparent-{{category.level}}" ng-if="category.children">
        <div ng-repeat="category in category.children" class="level-{{category.level}} accordion-item row">
            <h2 class="accordion-header col" ng-click="t['CD'+category.id]=!t['CD'+category.id];Locdethi(category,t)">
              <button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#a{{category.id}}"  ng-class="{'collapsed': !localStorage.CDDTActive['CD'+category.id]}">
              <span class="me-2 badge rounded-pill bg-warning">{{category.id |FCChudecon:RChude}}</span> <span class="me-2 badge rounded-pill bg-info">{{category|FSTailieu:Dethigoc}}</span>{{category.Toc}}. {{category.Tenchude}} 
          </button>
            </h2>
...
<ul class="dropdown-menu">
            ...
            <div id="a{{category.id}}" class="accordion-collapse collapse" data-bs-parent="#accordionparent-{{category.level-1}}" ng-class="{'show': localStorage.CDDTActive['CD'+category.id]}">
      ...
      <div class="accordion" id="accordionparent">
        ...
        <button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#a{{category.id}}"  ng-class="{'collapsed': !localStorage.CDDTActive['CD'+category.id]}">
          ...
          <div id="a{{category.id}}" class="accordion-collapse collapse" data-bs-parent="#accordionparent" ng-class="{'show': localStorage.CDDTActive['CD'+category.id]}">
            <div class="accordion-body" ng-include="'categoryTree'">{{category.Tenchude}} </div>
          </div>
        </div>
      </div>
