    ...
    <div class="row">
     <div class="col-sm-6">
      <div class="input-group mb-3">
       <span class="input-group-text">Tài Liệu Nguồn</span>
       ...
       <span class="input-group-text" ng-click="Dethi.idTailieu=''"> <i class="fas fa-sync-alt" ng-click="ResetCauhoi();dtsearch={}"></i> </span> <span class="input-group-text"><span class="badge bg-info me-2">{{RC.length}}</span> Câu Hỏi</span>
      </div>
      <!--
     <div class="row mb-3">
        ...
        <div class="d-flex">
         <div class="table mt-3 ghichu">
          <table class="table table-centered table-nowrap mb-0 rounded text-center table-bordered table-hover">
           <thead class="thead-light">
            <tr>
             <th style="width: 10%;"><div class="d-flex justify-content-center"><div class="dropdown position-static">
                  <div class="dropdown-toggle" data-bs-toggle="dropdown" data-bs-auto-close="outside">
...
<input class="form-control" type="text" ng-model="dtsearch.Cauhoi">
           ...
           <tbody>
            <tr ng-repeat="ch in RC = (RACauhoi |FMT:Dethi.idTailieu:'idTL' | filter:{Trangthai:2} |filter:dtsearch | orderBy:propertyName:reverse)">
             <td style="width: 10%;" ng-click="AddToDethi(ch)">
             <i class="fas fa-plus-circle"></i>
             <div><span class="badge rounded-pill text-white" ng-class="ch.Dapan.type==0?'bg-primary':'bg-danger'">{{ch.Dapan.type==0?'Trắc Nghiệm':'Tự Luận'}}</span></div>    
                </td>
         ...
         <div ng-repeat="ch in RCHDT | orderBy:'$index':true">
          <div class="card mb-3">
           <div class="card-header text-white bg-primary p-2 d-flex">
            ...
            <div class="me-2 d-flex"><input class="form-control w-25" ng-model="ch.Diem" ng-if="FlagSD" /><span ng-if="!FlagSD">{{ch.Diem}}</span> <span class="mx-2 my-auto">Điểm</span>
                <i class="fas fa-edit text-white me-2 my-auto" ng-click="FlagSD=!FlagSD" ng-show="!FlagSD"></i>
                <i class="fas fa-save text-white me-2 my-auto" ng-click="Suadiem(ch);FlagSD=!FlagSD" ng-show="FlagSD"></i>
               </div>
            <div class="ms-auto">
                <i class="fas fa-minus-circle text-white" ng-click="RemoveFromDethi(ch)"></i>
               </div>  
           </div>
           <div class="card-body">
   ...
   <div class="modal-footer">
    {{Dethi.Cauhoi}}
    <span class="input-group-text">Đã Chọn <span class="badge bg-primary mx-2">{{Dethi.idCH.length||'0'}}</span> Câu hỏi </span>
    <button type="button" class="btn btn-primary" class="btn btn-secondary" data-bs-dismiss="modal">Lưu</button>
   </div>
  </div>
 </div>
</div>   
