var app = angular.module('Site', ['ui.filters', 'localytics.directives', 'ngSanitize', 'ui.tinymce','ngCookies','thatisuday.dropzone']);
...
//    $scope.SNT={'from':'','to':''};
    ...
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/caidats?page[offset]=0&page[limit]=*", {
     ...
     //   console.log(data);
        ...
        $scope.Phongban = (data.find(result => result.id === 16)).Dulieu;
  ...
  $scope.UpdateCaidat = function (Caidat, Dulieus) {
      console.log(Dulieus);
//              var arr = [];
//        Object.keys(Dulieus).forEach(function(key)
//        {
//            arr.push(Dulieus[key]);
//        }); 
//      console.log(arr);
    var data = {
      "Title": Caidat.Title,
 ...
 $scope.CreateNhanVien = function (dulieu) {
    let Profile = {Hoten:'',Congty:'',Khoi:'',Phongban:'',Bophan:'',Vitri:'',MaNV:'',Ngaysinh:'',Gioitinh:'',CMND:'',SDT:'',Diachi:'',Datein:'',Dateout:'',Email:'',Zalo:'',Fb:''} 
    var data = { };
    // console.log(data);
    $http.post("", data, {
        headers: $scope.headers
  ...
  $scope.UpdateNhanvien = function(dulieu) {
     // console.log(dulieu);
    let Profile = {Hoten:dulieu.Profile.Hoten,Congty:dulieu.Profile.Congty,Khoi:dulieu.Profile.Khoi,Phongban:dulieu.Profile.Phongban,Bophan:dulieu.Profile.Bophan,Vitri:dulieu.Profile.Vitri,MaNV:dulieu.Profile.MaNV,Ngaysinh:dulieu.Profile.Ngaysinh,Gioitinh:dulieu.Profile.Gioitinh,CMND:dulieu.Profile.CMND,SDT:dulieu.Profile.SDT,Diachi:dulieu.Profile.Diachi,Datein:dulieu.Profile.Datein,Dateout:'',Email:dulieu.email,Zalo:'',Fb:''}   
    var data = {"groups": ["2"],Profile:JSON.stringify(Profile)};
    $http.patch("https://tazagroup.vn/api/index.php/v1/users/" + dulieu.id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
          $scope.OninitNhanVien(); 
      }, function (res) {
        console.log(res);
      }); 
  } 
  
  $scope.CapnhatNhanvien = function (dulieu) {
      console.log(dulieu);
      
//    angular.forEach($scope.RListNV, function (v) {  
//        if(v.id!=72)
//            {
//            let Profile = {Hoten:v.tencu,Congty:1,Khoi:'',Phongban:v.idPhongban,Bophan:0,Vitri:v.idVitri,MaNV:v.MaNV,Ngaysinh:v.Ngaysinh,Gioitinh:v.Gioitinh,CMND:v.CMND,SDT:v.SDT,Diachi:v.Diachi,Datein:v.Datein,Dateout:'',Email:v.email,Zalo:'',Fb:''};
//                  //  console.log(Profile);
//         let data = {"groups": ["2"],Profile:JSON.stringify(Profile)};
//    $http.patch("https://tazagroup.vn/api/index.php/v1/users/" + v.id, data, {
//        headers: $scope.headers
//      })
//      .then(function (res) {
//      }, function (res) {
...
//    let Profile = {Hoten:dulieu.tencu,Congty:1,Khoi:'',Phongban:dulieu.idPhongban,Bophan:1,Vitri:dulieu.Vitri,MaNV:dulieu.MaNV,Ngaysinh:dulieu.Ngaysinh,Gioitinh:dulieu.Gioitinh,CMND:dulieu.CMND,SDT:dulieu.SDT,Diachi:dulieu.Diachi,Datein:dulieu.Datein,Dateout:'',Email:dulieu.email,Zalo:'',Fb:''}   
//      console.log(Profile);
//    var data = {"groups": ["2"],Profile:JSON.stringify(Profile)};
//    $http.patch("https://tazagroup.vn/api/index.php/v1/users/" + dulieu.id, data, {
//        headers: $scope.headers
...
//      });
 ...
 $scope.editNhanvien = function(dulieu)
 {
     $scope.Nhanvien = dulieu;
     $scope.Nhanvien.Title = "Cập Nhật Nhân Viên";
     $scope.Nhanvien.CRUD = 1;
...
$scope.editDauviec = function (data) {
 ...
 $scope.ReadGviec = function () {
  ...
  //Tài Liệu Begin 
  $scope.LoaiTM =[
      {id:1,Title:'Lộ Trình',Thumuc:'Lotrinh'},
      {id:2,Title:'Chính Sách',Thumuc:'Chinhsach'},
      {id:3,Title:'Biên Bản',Thumuc:'Bienban'},
      {id:4,Title:'Đầu Mối',Thumuc:'Daumoi'},
      {id:5,Title:'Quy trình Các Phòng Ban',Thumuc:'Cacphongban'},
      {id:6,Title:'Quy trình Từng Phòng Ban',Thumuc:'Tungphongban'},
      {id:7,Title:'Thông Báo Công Ty',Thumuc:'Thongbao'},
      {id:8,Title:'Biểu Mẫu',Thumuc:'Bieumau'},
      {id:9,Title:'Tuyển Dụng',Thumuc:'Tuyendung'},
      {id:10,Title:'Đào Tạo',Thumuc:'Daotao'},
      {id:11,Title:'Chế Độ Phúc Lợi',Thumuc:'Phucloi'}
  ]
  $scope.loadFile = function (files) {
    $scope.$apply(function () {
      $scope.selectedFile = files[0];
...
//Kế Hoạch End  
  ...
  // Từng Phòng Ban Begin
$scope.OninitTungphongban = function (idTM) {
   // $scope.CurrentTM = $filter('FCustom')(idTM,'Title',$scope.LoaiTM);
    $scope.CurrentTM = $scope.LoaiTM.find(result => result.id === idTM);
    console.log($scope.CurrentTM);
    console.log(idTM);
    $scope.Oninit();
    $scope.ReadTungphongban(idTM);
    $scope.Tungphongban = {
      'CRUD': 0,
      'Title':"Thêm Mới"
    };
  ...
  $scope.CreateTungphongban = function (dulieu) {
    var data = {
      "idTM": $scope.CurrentTM.id,
      "idCty": dulieu.idCty,
      "idKhoi": dulieu.idKhoi,
      "idPB": dulieu.idPB,
      "idBP": dulieu.idBP,
      "idVitri": dulieu.idVitri,
      "Tieude": dulieu.Tieude,
    ...
    $http.post("https://tazagroup.vn/api/index.php/v1/hrms/tailieus", data, {
      ...
      .then(function (res) {
        console.log(res);
        $scope.OninitTungphongban($scope.CurrentTM.id);
  ...
  $scope.ReadTungphongban = function () {
    $http.get("https://tazagroup.vn/api/index.php/v1/hrms/tailieus?filter[published]=0&filter[idTM]="+$scope.CurrentTM.id+"&page[offset]=0&page[limit]=*", {
        headers: $scope.headers
      })
      .then(function (res) {
        $scope.RATungphongban = [];
        var data = getdata(res.data.data);
        angular.forEach(data, function (v) {
          v.Ngaytao = new Date(v.Ngaytao);
          v.File = JSON.parse(v.File)||[];
          v.Bieumau = JSON.parse(v.Bieumau)||[];
          $scope.RATungphongban.push(v);
        });
      });

  }
  $scope.UpdateTungphongban = function (dulieu) {
    var data = {
      "idTM": $scope.CurrentTM.id,
      "idCty": dulieu.idCty,
      "idKhoi": dulieu.idKhoi,
      ...
      "idVitri": dulieu.idVitri,
      "Tieude": dulieu.Tieude,
      "File": JSON.stringify($scope.TPBFile),
      "Bieumau": JSON.stringify($scope.TPBBieumau),
      "Ghichu": dulieu.Ghichu,
      "idTao": $scope.idUser
    };
    console.log(data);
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/tailieus/" + dulieu.id, data, {
        headers: $scope.headers
      })
      .then(function (res) {
        console.log(res);
  ...
  $scope.DeleteTungphongban = function (dulieu) {
    var data = {
      "published": 1
    };
    $http.patch("https://tazagroup.vn/api/index.php/v1/hrms/tailieus/" + dulieu.id, data, {
  ...
  $scope.editTungphongban = function (data) {
    $scope.Tungphongban = data;
    $scope.Tungphongban.idCty = data.idCty.toString();
    $scope.Tungphongban.idKhoi = data.idKhoi.toString();
    $scope.Tungphongban.idPB = data.idPB.toString();
    $scope.Tungphongban.idBP = data.idBP.toString();
    $scope.Tungphongban.idVitri = data.idVitri.toString();
    $scope.Tungphongban.CRUD = 1;
    $scope.Tungphongban.Title = "Chỉnh Sửa";
  } 
   $scope.removeTungphongban = function (data) {
    console.log(data);
    $scope.Tungphongban = data;
    $scope.Tungphongban.CRUD = 0;
  }
  $scope.resetTungphongban = function()
  {  
      $scope.Tungphongban = {
      'CRUD': 0,
      'Title':"Thêm Mới"
  ...
  $scope.dzOptions = {
    method: 'POST',
    url: '/index.php?option=com_hrms&task=Tailieu.uploadhrm&format=raw',
 ...
 $scope.dzOptions1 = {
  ...
  $scope.dzCallbacks = {
    ...
    'sending': function (file, xhr, formData) {
            formData.append("data",$scope.CurrentTM.Thumuc);
    },     
    'success': function (file, res) {
      $scope.TPBFile.push(JSON.parse(res));
        console.log($scope.TPBFile);
            console.log(res);
    },
  ...
  $scope.dzCallbacks1 = {
    ...
    'success': function (file, res) {
      $scope.TPBBieumau.push(JSON.parse(res));
        console.log($scope.TPBBieumau);
            console.log(res);
    },   
    'sending': function (file, xhr, formData) {
            formData.append("data",$scope.CurrentTM.Thumuc);
  ...
  $scope.Tailen = function()
    {$scope.dzMethods.processQueue(); 
     $scope.dzMethods1.processQueue();
    } 
  $scope.Tailen1 = function()
    {$scope.dzMethods1.processQueue();  }
