          ...
          <table class="table dauviec dauviec-sm table-centered table-nowrap mb-0 rounded text-center table-bordered table-hover">
            ...
            <tbody style="height: 60vh!important">
              <tr ng-repeat="rd in RATungphongban | filter:timkiem| orderBy:propertyName:reverse">
                ...
                <td ng-show="tieude.td2" style="width: 15%;">
                      <div class="btn-group position-static" ng-if="rd.File.length!=0">
                          <span data-bs-toggle="dropdown" data-bs-auto-close="outside" class="position-relative">
                           <i class="fas fa-file-pdf text-danger fs-2"></i> 
                           <span class="badge bg-info" style="position: absolute;top: -5px;right: -5px;">{{rd.File.length}}</span>   
                          </span>
                      <ul class="dropdown-menu">
                        <li>
                          <div ng-repeat="x in rd.File" class="d-flex"> 
                              <span class="dropdown-item text-info" data-bs-toggle="modal" data-bs-target="#PDFView" ng-click="SetPDF(x)"><span class="badge bg-info">{{$index+1}}</span>   {{x.name}} </span> 
                              <i class="fas fa-minus-circle me-2 my-auto text-danger" ng-click=""></i>
                            </div>
                        </li>
                      </ul>
                    </div>
                ...
                <td ng-show="tieude.td2" style="width: 15%;">
           <div class="btn-group position-static" ng-if="rd.Bieumau.length!=0">
          <span data-bs-toggle="dropdown" data-bs-auto-close="outside" class="position-relative">
                           <i class="fas fa-file-pdf text-danger fs-2"></i> 
                           <span class="badge bg-info" style="position: absolute;top: -5px;right: -5px;">{{rd.Bieumau.length}}</span>   
                          </span>     
                      <ul class="dropdown-menu">
                        <li>
                          <div ng-repeat="x in rd.Bieumau" class="d-flex"> <span class="dropdown-item text-info" data-bs-toggle="modal" data-bs-target="#PDFView" ng-click="SetPDF(x)"><span class="badge bg-info">{{$index+1}}</span>  {{x.name}} </span> <i class="fas fa-minus-circle me-2 my-auto text-danger" ng-click=""></i></div>
                        </li>
                      </ul>
                    </div>         
                    </td>
