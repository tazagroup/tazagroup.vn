...
namespace Joomla\Component\Hrms\Api\Serializer;
 ...
 * @since  4.0.0
 */
class HrmsSerializer extends JoomlaSerializer
{
	...
	public function languageAssociations($model)
	{
		...
		foreach ($model->associations as $association)
		{
			$resources[] = (new Resource($association, $serializer))
				->addLink('self', Route::link('site', Uri::root() . 'api/index.php/v1/hrms/hrms/' . $association->id));
		}
	...
	public function category($model)
	{
		...
		$resource = (new Resource($model->catid, $serializer))
			->addLink('self', Route::link('site', Uri::root() . 'api/index.php/v1/hrms/categories/' . $model->catid));
